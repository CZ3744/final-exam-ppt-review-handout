...
DEFAULT_DOCUMENT_TITLE_SUFFIX = "考前复习讲义版"
BOILERPLATE = [
...
def discover_pptx(path: Path, recursive: bool = False) -> tuple[list[Path], list[Path]]:
    ...
    The function is intentionally tolerant: missing paths and unrelated files
    return empty results instead of raising, so CLI commands can write reports.
    """
    if not path.exists():
        return [], []
    if path.is_file():
...
def is_noise(text: str, custom: list[str] | None = None) -> bool:
    ...
    if not t:
        return True
    if any(re.match(p, t) for p in BOILERPLATE):
        return True
...
def deck_to_compact_md(deck: dict, max_table_rows: int = 8) -> str:
    lines = [
    ...
    max_table_rows = max(1, int(max_table_rows or 8))
    for slide in deck["slides"]:
        lines.append(f"## 第 {slide['index']} 页：{slide['title']}")
        lines.append(f"- 检测类型：{slide['detected_role']}")
        if slide.get("image_count"):
            lines.append(f"- 图片/图示数量：{slide['image_count']}（如该页主要靠图片表达，请结合原 PPT 或视觉模型复核）")
        if slide.get("texts"):
            lines.append("- 文字内容：")
        ...
        if slide.get("tables"):
            ...
            for i, table in enumerate(slide["tables"], start=1):
                ...
                if len(rows) > max_table_rows:
                    lines.append(f"    - ……其余 {len(rows) - max_table_rows} 行略，详见 slides.json")
        if slide.get("notes"):
            lines.append("- 备注：")
...
def image_heavy_warning(deck: dict) -> str | None:
    slide_count = deck.get("slide_count", 0) or 0
    if slide_count <= 0:
        return None
    heavy = [s for s in deck.get("slides", []) if s.get("detected_role") == "figure-heavy" or (s.get("image_count", 0) >= 3 and not s.get("texts"))]
    if len(heavy) / slide_count >= 0.3:
        return f"Image-heavy deck: {len(heavy)}/{slide_count} slides rely heavily on images. Ask the calling LLM or a vision/OCR step to review the original PPT."
    return None


def input_error_record(input_path: Path) -> dict | None:
    if not input_path.exists():
        return {"source_file": str(input_path), "chapter_title": "Input path not found", "slide_count": 0, "warnings": [], "errors": [f"Input path does not exist: {input_path}"]}
    if not input_path.is_file() and not input_path.is_dir():
        return {"source_file": str(input_path), "chapter_title": "Invalid input path", "slide_count": 0, "warnings": [], "errors": [f"Input path is neither a file nor a directory: {input_path}"]}
    return None
...
def extract_cmd(args) -> int:
    ...
    records = []
    if err := input_error_record(input_path):
        records.append(err)
            ...
            records.append(
                {
                    ...
                    "chapter_title": "No PPTX/PPTM files found",
                    "slide_count": 0,
                    "warnings": [],
    ...
    max_table_rows = config.get("max_table_rows_in_summary", 8)
    for ppt in files:
        ...
        try:
            ...
            if warning := image_heavy_warning(deck):
            ...
            out_md.write_text(deck_to_compact_md(deck, max_table_rows=max_table_rows), encoding="utf-8")
...
def add_title(target, title: str, suffix: str = DEFAULT_DOCUMENT_TITLE_SUFFIX):
...
def validate_handout_schema(handout: dict) -> list[str]:
    errors: list[str] = []
    missing = sorted(HANDOUT_REQUIRED_KEYS - set(handout))
    if missing:
...
def handout_to_docx(handout: dict, path: Path, layout: str = "review-margin", title_suffix: str = DEFAULT_DOCUMENT_TITLE_SUFFIX):
    errors = validate_handout_schema(handout)
    if errors:
        raise ValueError("; ".join(errors))
    doc = setup_doc(layout)
    target = prepare_content_target(doc, layout)
    add_title(target, handout.get("chapter_title", "未命名章节"), suffix=title_suffix)
    ...
    if handout.get("generated_by_fallback"):
        warn = target.add_paragraph()
        r_warn = warn.add_run("提示：本文档由 fallback 规则模式生成，只适合作为 smoke test 或粗略草稿，高质量结果仍需调用方 LLM 语义整理。")
        set_font(r_warn, "黑体", 10.5, True)
    for title, items in [("一、本章复习目标", handout.get("review_goals", [])), ("二、本章知识框架", handout.get("knowledge_framework", []))]:
    ...
    if handout.get("image_heavy_slides"):
        ...
        add_bullets(target, [f"第 {i} 页图示信息较多，建议结合原 PPT 或视觉模型复核。" for i in handout["image_heavy_slides"]])
...
def handout_json_files(analysis: Path) -> list[Path]:
    if analysis.is_file():
        return [analysis]
...
def render_cmd(args) -> int:
    config = load_config(args.config)
    title_suffix = config.get("document_title_suffix", DEFAULT_DOCUMENT_TITLE_SUFFIX)
    analysis = Path(args.analysis).expanduser().resolve()
    out = Path(args.output).expanduser().resolve()
    files = handout_json_files(analysis)
    records = []
    if not files:
        records.append({"source_file": str(analysis), "chapter_title": "No handout JSON files found", "slide_count": 0, "warnings": [], "errors": ["No *.handout.json files found. This command intentionally refuses raw slides.json files."]})
    ...
    for jf in files:
        ...
        try:
            ...
            schema_errors = validate_handout_schema(handout)
            if schema_errors:
                raise ValueError("; ".join(schema_errors))
            stem = safe_name(handout.get("chapter_title", jf.stem))
            docx = out / "docx" / f"{stem}_复习讲义版.docx"
            handout_to_docx(handout, docx, layout=args.layout, title_suffix=title_suffix)
            rec.update({"chapter_title": handout.get("chapter_title", jf.stem), "slide_count": handout.get("slide_count", 0), "docx": str(docx)})
            if handout.get("generated_by_fallback"):
                rec["warnings"].append("Generated by fallback rule mode; semantic LLM review is still recommended.")
            if args.export_pdf:
...
def fallback_handout(deck: dict) -> dict:
    ...
    return {
        ...
        "confusing_points": ["对名称相近的材料、工艺或技术，应从作用、适用对象和优缺点进行区分。"],
...
def build_cmd(args) -> int:
    ...
    if not files:
        if not records:
            records.append({"source_file": str(input_path), "chapter_title": "No PPTX/PPTM files found", "slide_count": 0, "warnings": [], "errors": ["No supported .pptx/.pptm files found."]})
        write_report(workspace, records)
        return 1
    max_table_rows = config.get("max_table_rows_in_summary", 8)
    for ppt in files:
        try:
            ...
            if args.keep_intermediate:
                ...
                (inter / f"{stem}.compact.md").write_text(deck_to_compact_md(deck, max_table_rows=max_table_rows), encoding="utf-8")
...
def write_report(out: Path, records: list[dict]):
    ...
    lines += ["## Records", ""]
    for r in records:
        line = f"- {r.get('chapter_title')}: {r.get('slide_count')} slides"
        if r.get("docx"):
            line += f"; DOCX: {r['docx']}"
        if r.get("pdf"):
            line += f"; PDF: {r['pdf']}"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Final-exam PPTX/PPTM review handout skill")
    sub = parser.add_subparsers(dest="command")

    p = sub.add_parser("extract", help="Extract PPTX/PPTM into slides.json and compact.md")
    p.add_argument("--input", required=True)
    p.add_argument("--workspace", "--output", dest="workspace", required=True)
    p.add_argument("--config")
    p.add_argument("--recursive", action="store_true", help="Recursively scan input directories.")
    ...
    p.add_argument("--layout", choices=LAYOUT_CHOICES, default="review-margin", help="DOCX layout. review-margin leaves a right-side annotation area.")
    ...
    p.add_argument("--export-pdf", action="store_true")
    p.add_argument("--zip-word", action="store_true")
    p.add_argument("--layout", choices=LAYOUT_CHOICES, default="review-margin")
    p.add_argument("--recursive", action="store_true", help="Recursively scan input directories.")
    return parser
